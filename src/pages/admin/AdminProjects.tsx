...
const empty = {
  ...
  images: [] as string[], completion_date: "", published: false,
};
        ...
        <div className="grid gap-4">
          {items.map((p) => (
            <Card key={p.id} className="p-4 flex gap-4 items-start">
              {p.images?.[0] && <img src={p.images[0]} alt="" className="w-24 h-24 object-cover rounded-lg shrink-0" />}
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {p.completion_date ? formatDate(p.completion_date, lang) : formatDate(p.created_at, lang)}
                  {p.images?.length > 1 && ` • ${p.images.length} ${lang === "ar" ? "صور" : "images"}`}
                </p>
              </div>
              <div className="flex gap-2 shrink-0">
                <Button size="icon" variant="outline" onClick={() => setEditing({
                  ...
                  images: p.images || [], completion_date: p.completion_date || "",
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {editing && (
            <div className="space-y-4">
              ...
              <div>
                <Label>{t("crud.completionDate")}</Label>
                <Input type="date" value={editing.completion_date} onChange={(e) => setEditing({ ...editing, completion_date: e.target.value })} />
              </div>
              <div>
                <Label>{t("crud.images")}</Label>
                <MultiImageUpload value={editing.images} onChange={(urls) => setEditing({ ...editing, images: urls })} folder="projects" />
              </div>
              <div>
                <Label>{t("crud.descriptionAr")} *</Label>
                <Textarea rows={5} value={editing.description_ar} onChange={(e) => setEditing({ ...editing, description_ar: e.target.value })} dir="rtl" />
              </div>
              <div>
